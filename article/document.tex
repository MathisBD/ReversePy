...
3. extracting the VM state/recognizing the VM model (pointers, instr blocks, frame changes)
	-> list the heuristics for each pointer (we look at register values between opcodes, i.e. just before the fetch instruction)
		ip :
			alignment >= 0x2
